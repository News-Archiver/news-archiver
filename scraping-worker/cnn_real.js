...
connection.connect((err) => {
  ...
  let createCNN = `create table if not exists cnn_real(
...
async function saveToDB(headline, link, date, month, imgLink, imgAlt) {
  var sql = `INSERT INTO cnn_real (headline, link, date, month, imglink, imgalt) VALUES ("${headline}", "${link}", "${date}", "${month}", "${imgLink}", "${imgAlt}");`;
  console.log(sql);
...
async function removeDuplicate() {
  let deleteDuplicate =
    "DELETE t1 FROM news.cnn_real t1 INNER JOIN news.cnn_real t2 WHERE t1.id < t2.id AND t1.link = t2.link;";
